...
const apiKey = "1f5de9bd56904cb281d915f1fdb805fa";
...
async function fetchRandomNews(){
    try{
        ...
        console.error("error to fetch random news", error)
        return[];
    }
}
...
searchButton.addEventListener("click", async () => {
    ...
    if(query !== ""){
        try{
            ...
            console.log("error fitching news by query", error)
        }
}
})
...
(async()=>{
    try{ const articles = await fetchRandomNews()
    displayBlogs(articles)
}catch(error){
    console.error("error to fetch random news", error)
    return[]
